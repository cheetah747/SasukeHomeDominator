...
class MainActivity : AppCompatActivity() {
    ...
    fun changeBtnState(cardView: View, isSelected: Boolean){
        ...
        (cardView.findViewById<CardView>(R.id.cardContainer) as LinearLayout).setBackgroundColor(if (isSelected) resources.getColor(R.color.colorPrimary,null) else Color.WHITE)
    }

    fun setListeners() {
        arrayOf(lockScreenCard, screenShotCard).forEach {
            it.setOnClickListener {
                PreferHelper.getInstance().setString(StaticVar.KEY_SELECTED_ITEM,
                    when (it) {
                ...
                Snackbar.make(root,"【${it.findViewById<TextView>(R.id.cardText).text}】に設定しました",Snackbar.LENGTH_SHORT).show()
                initUI()//再刷新一下页面
            }
        }
